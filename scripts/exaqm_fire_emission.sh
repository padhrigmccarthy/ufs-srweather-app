...
if [ -e "${DCOMINfire}/${aqm_fire_file_fn}" ]; then
  ...
  for ihr in {0..23}; do
    ...
    elif [ -s `ls ${DCOMINfire}/${yyyymmdd_dn}/rave/${FILE_curr}` ]; then
      echo "NOTE: Copying pre-existing file ${DCOMINfire}/${yyyymmdd_dn}/rave/${FILE_curr} to ${FILE_curr} ..."
      cp -p ${DCOMINfire}/${yyyymmdd_dn}/rave/${FILE_curr} ${FILE_curr}
